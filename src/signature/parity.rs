...
use crate::signature::{
    ...
    utils::{normalize_v_to_byte, to_eip155_v},
};
...
impl TryFrom<u64> for Parity {
    ...
    fn try_from(value: u64) -> Result<Self, Self::Error> {
        match value {
            ...
            27 | 28 => Ok(Self::NonEip155(value == 28)),
            35..=u64::MAX => Ok(Self::Eip155(value)),
...
impl Parity {
    ...
    pub const fn inverted(&self) -> Self {
        match *self {
            ...
            Self::Eip155(v @ 1..=34) => Self::Eip155(if v & 1 == 0 { v - 1 } else { v + 1 }),
    ...
    pub unsafe fn invert_simd(parities: &mut [Self]) {
            ...
            *parity = match *parity {
                ...
                Self::Eip155(v) => {
                    if v == 0 {
                        ...
                        let v_vec = vdupq_n_u64(v);
                        let one = vdupq_n_u64(1);
                        let result = veorq_u64(v_vec, one);
                        Self::Eip155(vgetq_lane_u64(result, 0))
                    }
                }
            };
        }
    ...
    pub const fn strip_chain_id(&self) -> Self {
        match *self {
            Self::Eip155(v) => Self::NonEip155(v & 1 == 1),
            this => this,
        }
    }
...
mod test {
    ...
    fn test_simd_invert_batch() {
        ...
        unsafe { Parity::invert_simd(&mut parities) };

        assert_eq!(parities[0], Parity::Eip155(1));
        assert_eq!(parities[1], Parity::Eip155(13));
        assert_eq!(parities[2], Parity::Eip155(58));
        assert_eq!(parities[3], Parity::NonEip155(false));
        assert_eq!(parities[4], Parity::Parity(true));
    }
}
